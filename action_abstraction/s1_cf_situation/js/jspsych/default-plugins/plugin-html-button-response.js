      ...
      trial(display_element, trial) {
          ...
          for (var i = 0; i < trial.choices.length; i++) {
              display_element
                  ...
                  var choicehtml = btn_el.innerHTML
                  after_response(choice, choicehtml);
              });
          }
          // store response
          var response = {
          ...
          const end_trial = () => {
              ...
              var trial_data = _.extend({}, _.omit(trial, 'on_finish', 'type', 'data'), {
                  rt: response.rt,
                  stimulus: trial.stimulus,
                  response: response.button,
                  responsehtml: response.button_html
              });
              // clear the display
          ...
          function after_response(choice, choicehtml) {
              // measure rt
              var end_time = performance.now();
              var rt = Math.round(end_time - start_time);
              response.button = parseInt(choice);
              response.button_html = choicehtml;
              response.rt = rt;
