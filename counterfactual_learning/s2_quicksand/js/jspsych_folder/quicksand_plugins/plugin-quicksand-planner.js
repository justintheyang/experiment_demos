      ...
      const updateTrialPhase = () => {
        if (
          ...
          if (trial.feedback) {
            ...
            const num_quicksand = (path.filter((tile) => tile.isQuicksand).length);
  
            const bonusText = `You earned $${bonus} for this trial.`;
            if (num_quicksand < 1) {
              occluderText.innerHTML = `<p>Perfect 🤩</p><p>${bonusText}</p>`;
            } else if (num_quicksand < 2) {
              occluderText.innerHTML = `<p>Great job 😊</p><p>${bonusText}</p>`;
            } else if (num_quicksand < 4) {
