var jsPsychQuicksandEvalInstance = (function (jspsych) {
  "use strict";

  const info = {
    name: "quicksand-eval-instance",
    parameters: {
      spec1: {
  ...
  /**
   * **QUICKSAND-EVAL-INSTANCE**
   *
   * A jsPsych plugin for evaluating understanding of quicksand instances
   *
   * @author Justin Yang
   */
  class QuicksandEvalInstancePlugin {
    constructor(jsPsych) {
    ...
    trial(display_element, trial) {
      ...
      // Fully reveal gridworlds
      gridworld1.tiles.flat().forEach((tile) => (tile.isObserved = true));
      gridworld2.tiles.flat().forEach((tile) => (tile.isObserved = true));

      // Set colors based on quicksand value
      // TODO: we need to validate that the quicksand probabilities are 0 or 1 for this to work as intended.
      gridworld1.tiles.flat().forEach((tile) => {
        if (tile.probabilityOfQuicksand !== undefined) {
          tile.fillStyle = tile.probabilityOfQuicksand
      ...
      gridworld2.tiles.flat().forEach((tile) => {
        if (tile.probabilityOfQuicksand !== undefined) {
          tile.fillStyle = tile.isQuicksand
            ? gs.tile.colors.quicksand
            : gs.tile.colors.sand;
        }
      });
  ...
  QuicksandEvalInstancePlugin.info = info;

  return QuicksandEvalInstancePlugin;
})(jsPsychModule);
